import { GlobalData } from "../../common/config/GlobalConfig";
...
@ccclass
export default class BaseFunc extends cc.Component {
    ...
    static setStorage(key: string, value: any) {
        cc.sys.localStorage.setItem(GlobalData.gameName + key, value);
    }
    //获取本地数据
    static getStorage(key: string): any {
        return cc.sys.localStorage.getItem(GlobalData.gameName + key);
    }
    ...
    // 首字母变成大写
    static initialUpperCase(str: string): string {
    ...
    //             BaseFunc.loadImg(`texture/tips/${imgId}`, newNode.getChildByName("img"));
    //         } else {
